...
const OrderCard: React.FC<{ order: Order; onComplete: (id: string) => Promise<boolean>; currency: string }> = ({ order, onComplete, currency }) => {
  const [isCompleting, setIsCompleting] = useState(false);

  const handleComplete = async () => {
    setIsCompleting(true);

    const success = await onComplete(order.id);
  ...
  return (
    <div className={`bg-white rounded-lg shadow-lg overflow-hidden transition-all duration-500 ${isCompleting ? 'animate-swipe-out' : 'animate-fade-in'}`}>
      ...
      <button
        ...
        className="w-full bg-green-500 text-white font-bold py-3 hover:bg-green-600 transition-colors disabled:bg-gray-400"
      >
...
const WaiterDashboard: React.FC = () => {
  ...
  // Fetch once (Realtime handles updates after)
  useEffect(() => {
  ...
  // ✅ FIXED: only remove if DB update succeeds
  const handleComplete = async (orderId: string): Promise<boolean> => {
    try {
      const result = await completeOrder(orderId);

      // If your context doesn't return anything, assume success
