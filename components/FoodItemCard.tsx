...
const FoodItemCard: React.FC<FoodItemCardProps> = ({ item, orderType }) => {
  const { addToCart, currency } = useAppContext();
  const { isAvailable } = item;

  // Determine the price to display and add to cart
  const displayPrice = orderType === 'takeaway' && item.takeawayPrice > 0
    ? item.takeawayPrice
    : item.price;

  const handleAdd = () => {
    // Create a copy of the item with the correct price for this order type
    const itemForCart = {
      ...item,
      price: displayPrice, // override with the selected price
    };
    ...
    <div className={`relative bg-white rounded-lg shadow-lg overflow-hidden transform hover:scale-105 transition-transform duration-300 flex flex-col ${!isAvailable ? 'grayscale opacity-60' : ''}`}>
      ...
      <div className="p-4 flex flex-col flex-grow">
        ...
        <div className="flex justify-between items-center mt-auto">
          <span className="text-lg font-bold text-brand-primary">{currency} {displayPrice.toFixed(2)}</span>
          <button
            onClick={handleAdd}
